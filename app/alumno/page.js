...
const ALUMNO_NOMBRE = process.env.NEXT_PUBLIC_ALUMNO_NOMBRE || 'Alumno'
const ALUMNO_ID     = process.env.NEXT_PUBLIC_ALUMNO_ID     || '—'
...
export default function AlumnoPage() {
  ...
  useEffect(() => {
    fetch('/api/estado')
      ...
      .then(data => {
        ...
        const vm = data.vms?.find(v => v.id === ALUMNO_ID)
        if (vm?.estado) {
          const match = vm.estado.match(/\d+/)
          if (match) setPasoActual(parseInt(match[0]))
        }
      })
      .catch(() => {})

    fetch('/api/alumno/repo')
      .then(r => r.json())
  ...
  function pasoColor(n) {
    if (pasoActual === null) return 'var(--text3)'
    if (n < pasoActual)     return 'var(--accent4)'
    if (n === pasoActual)   return 'var(--accent)'
    return 'var(--text3)'
  }
  ...
  const completados = pasoActual !== null ? pasoActual - 1 : 0
    ...
    <>
      ...
      <div className="header">
        <div>
          ...
          <div className="header-sub">// {ALUMNO_NOMBRE} · {ALUMNO_ID}</div>
        </div>
      ...
      <div style={{ maxWidth: '900px', margin: '0 auto', padding: '32px' }}>
        ...
        <div style={{ marginBottom: '36px' }}>
          ...
          <div className="vm-card" style={{ borderTopColor: vmDot }}>
            ...
            <div style={{ paddingTop: '20px' }}>
              {vmLoading ? (
                <button className="btn btn-ghost" disabled style={{ width: '100%', padding: '18px', fontSize: '17px' }}>
                  ⏳ Procesando...
                </button>
              ) : vmStatus === 'archived' ? (
                <button className="btn btn-green" onClick={desarchivar} style={{ width: '100%', padding: '18px', fontSize: '17px' }}>
                  ▶ ENCENDER VM
                </button>
              ) : vmStatus === 'active' ? (
                <button className="btn btn-red" onClick={archivar} style={{ width: '100%', padding: '18px', fontSize: '17px' }}>
                  ⛔ APAGAR VM
                </button>
              ) : vmStatus === null ? (
                <button className="btn btn-ghost" disabled style={{ width: '100%', padding: '18px', fontSize: '17px' }}>
        ...
        <div style={{ marginBottom: '36px' }}>
          ...
          <div className="card">
            {lastCommit === null ? (
              <div style={{ fontFamily: 'var(--font-mono)', color: 'var(--text2)', fontSize: '16px' }}>
                Cargando...
              </div>
            ) : lastCommit === 'error' ? (
              <div style={{ fontFamily: 'var(--font-mono)', color: 'var(--accent2)', fontSize: '16px' }}>
